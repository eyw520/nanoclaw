...
// nothing — kill and restart on the next inbound. Sourced from config.ts
// (CONTAINER_CEILING_MS, read through envConfig) — NOT process.env directly:
// .env is parsed into envConfig, not injected into process.env, so a bare
// process.env read here silently used the 30min default regardless of .env.
export const ABSOLUTE_CEILING_MS = CONTAINER_CEILING_MS;
